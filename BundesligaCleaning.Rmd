...
not_real_name17 <- not_missing_bundesliga_17 %>% 
...
```{r}
weird_names17 <- last_names_bundesliga17 %>% 
...
```{r}
fifa17_standard_plus_table <- fifa17_bundesliga_standard_stats %>% 
...
not_real_name18 <- not_missing_bundesliga_18 %>% 
  filter(is.na(overall) == FALSE) %>% 
  select(-Player1, -player_name, -player_extended_name)
```
...
```{r}
weird_names18 <- last_names_bundesliga18 %>% 
...
```{r}
fifa18_standard_plus_table <- fifa18_bundesliga_standard_stats %>% 
...
fifa18_modeling <- fifa18_standard_plus_table %>% 
  ...
  rename(minutes_played_divided_by90 = X90s, Non_PK_G = G.PK, G_plus_A_per90 = G.A, G_minus_Pk_per90 = G.PK.1, G_plus_A_minus_PK_per90 = G.A.PK, Save_percent = Save., CS_percent = CS., Pk_Save_percent = Save..1, G_per90 = Gls.1, A_per90 = Ast.1 ) %>% 
  select(-Matches.x, -Matches.y, -Player1, -xG, -npxG, -xA, -npxG.1, -npxG.xA, -npxG.xA.1, -xG.1, -xA.1, -xG.xA, -Rk, -W.y) %>% 
  mutate(Nation = sub('.* ', '', Nation)) %>% 
  rename(W = W.x, D = D.x, L = L.x)
```
...
not_real_name19 <- not_missing_bundesliga_19 %>% 
  filter(is.na(overall) == FALSE) %>% 
  select(-Player1, -player_name, -player_extended_name)
```
...
```{r}
weird_names19 <- last_names_bundesliga19 %>% 
...
```{r}
fifa19_standard_plus_table <- fifa19_bundesliga_standard_stats %>% 
