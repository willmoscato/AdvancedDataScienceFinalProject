...
standardprem16 <- read.csv("standardprem16.csv")
...
fifa16_prem_full <- prem16 %>%
  ...
  select(Player, revision, position)
```


```{r}
standardprem16 <- standardprem16 %>% 
  mutate(Player1 = gsub(".*\\\\", "", Player)) %>% 
  mutate(Player = str_replace_all(Player1, "-", " "))
```

```{r}
fifa16_prem_standard_stats <- fifa16_prem_full %>% 
  inner_join(standardprem16, by = "Player") %>% 
  select(-Player1, -Pos) %>% 
  distinct(Player, .keep_all = TRUE)
```
...
fifa18_prem_full <- prem18 %>%
  ...
  distinct(Player, .keep_all = TRUE) %>% 
  select(Player, Nation, Squad, Age, revision, position, X90s, CrdY, CrdR, Fls)
```
