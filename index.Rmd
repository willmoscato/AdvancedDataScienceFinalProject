...
prem_ranger_test_def <- prem_ranger_test21 %>% 
  ...
  arrange(desc(.pred_TOTS)) %>% 
  head(5)  %>% 
    mutate(Role = ifelse(rank(.pred_TOTS, ties.method = c("first")) >= 4, "Starter", "Bench"), Position = 3)
```
...
kable(prem_tots_formatted, align = c(rep('c',12)))  %>% 
  ...
  row_spec(1:8, background = "green", color = "white") %>% 
  row_spec(9:15, background = "red", color = "white") 
```
