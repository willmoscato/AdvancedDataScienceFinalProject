...
```{r}
set.seed(494)
ligue1_split <- initial_split(ligue1_modeling_outfield, prop = .5, strata = "revision")
...
ligue1_lasso_recipe <- recipe(revision ~ . , data = ligue1_training) %>% 
  step_rm(Player, Nation, Squad, Born, minutes_played_divided_by90, G_per90, A_per90) %>%
