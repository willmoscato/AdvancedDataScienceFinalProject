...
author: "Will Moscato, Alex Denzler, Jake Greenberg, Michael Helton"
output:
  html_document:
    toc: TRUE
    toc_float : TRUE
...
\
We will now take you through the exploratory plots and the modeling stage of the project. Much like the data cleaning the process for each league is mostly the same so we will just go through the Premier League in depth and then show all the code and briefly mention the differences for the other leagues. 
\

\
First we added a year tag to the end of the player names in order to differentiate the same players from year to year when looking at what we got right and wrong. We also created the full modeling data set and made some of the characters factors so they can be used by the model.
\
...
```
\
Next we filtered out the players and stats we didn't want in the modeling. We used 19 as the minutes_played_divided_by90 because that is half of the games, thus those people should be the starters. We have a bunch of extra variables that we got rid of because we thought we were going to try to do goalkeepers too, but there wasn't enough data.
\
```{r premmod3}
...
```
\
Here is our first exploratory plot looking at the difference in the stats between training and testing.
\
...
```
\
Before diving into modeling, we explored the data to observe basic trends.
\
...
```
\
First, we looked at the proportion of Premier League cards that are given the TOTS designation. Below, we see that a select few cards are given the TOTS designation.
\
```{r }
...
\
We then made a density plot of the goalscoring between tots and not to see how important goals are.
\
...
\
Then we made a density plot of table position between tots and not to see if how good the team of the player is important.
\
...
\
Then we looked at how much tots played vs non tots to see if it is only starters that get the cards.
\
...
prem_mppn_density <- prem_modeling %>% 
  ...
  geom_density(alpha = 0.5) +
  scale_fill_manual(values = c("TOTS" = "blue", "Normal" = "gold")) +
  labs(x = "Total Minutes Played Divided by 90 (Full Games Played)", y = "Density", title = "Density Plot of Premier League Minutes Played (Per 90 Minutes) by Revision Status", subtitle = "Players who accrue more minutes per contest are more likely to be picked for Premier League TOTS") + 
...
\
Lastly, we looked at the positions of the players.
\
...
\
We then made the recipe for the model. We decided to use a random forest model because it made the most sense for predicting a binary response variable. We had also tried using a lasso model, but they were consistently worse for each league. We removed some redundant variables and some variables that are a little too specific to the certain player. We then decided to upsample the data to include more team of the seasons in the training data. This is where the different leagues really differ. We did some guess and check for each league in order to find the best upsampling that made the best model.
\
...
```
\
Then we created the cross validation and tuned the model for the min n and mtry. Then we looked at the roc_auc and accuracy for the crossed validated model.
\
```{r premranger3}
set.seed(494)
...
```
\
Next we selected the best tuning using accuracy and finalized the workflow.
\
```{r premranger4}
...
```
\
We used the explainer to created a variable importance plot as this felt like one of the questions we were really trying to answer.
\
...
\
Then we fit the model on the training data and looked at accuracy and roc_auc.
\
```{r premranger6}
...
```
\
Then we made a confusion matrix of the model on the testing data to see how well the model was working. This is what we mostly used to decide if the model was good or if something (like the upsampling) needed to change.
\
```{r premconfuse1}
...
\
Then we fit the model on the testing data in way that it would be usable to us and that we could see individual predicted probabilities. Then we made another confusion matrix and colored it and made it look nice and presentable.
\
...
\
Then we looked at what players the model was predicting incorrectly to better understand the pitfalls of the model.
\
...
```
\
Then we created a modeling data set for the current season.
\
```{r 21prem}
...
```
\
And we fit our model to it in order to see how well the model preformed on new data.
\
```{r 21premranger1_1}
...
```
\
We then used that to show our predicted team of the season for each league this year. We used the top five predicted probabilities for each position group because the model did not predict a lot of team of the seasons, but it tended to order things well.
\
...
```{r bundesligaconfuse1, , include= FALSE}
...
```{r bundesligaranger7,message = FALSE, , include= FALSE}
...
```{r bundesligaconfuse2, , include= FALSE}
bundesliga_ranger_test %>% 
  conf_mat(revision, .pred_class)
```

```{r bundesligawrong, , include= FALSE}
bundesliga_ranger_test %>% 
  filter(revision != .pred_class)
```
...
kable(serie_a_tots_formatted, align = c(rep('c',11)))  %>% 
  ...
  row_spec(11:15, background = "red", color = "white") 
```
# Kevin De Bruyne in the Models
\
We used a single player to illustrate the difference in the models between leagues and show how the different predictions are made.
\
...
In conclusion, we cleaned the data (mostly names), made five separate models, and looked at how those models compared to each other. The most difficult part of this task was making sure we could join the data sets and making the upsampling of the models correct (depending on the size of the leagues and the number of tots players it could vary a lot). We should mention that the knitted version of this does not match the version in rstudio, so in our final product we saved the images of the outputs that we wanted. Therefore, I'm not sure how reproducalbe this actually is, but all the code should be right.
